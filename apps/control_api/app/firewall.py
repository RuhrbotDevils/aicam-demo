...
def _render_wifi_egress_block(wifi_lock_iface: str | None) -> str:
    """Receive-only egress lock for the field-wifi interface.

    When a field-wifi profile is active, the camera must not transmit
    onto the field network beyond what L2 association needs. 802.11
    auth/assoc, the WPA/EAPOL handshake, and ARP are link-layer (not
    IP) traffic and are unaffected by this filter chain, so the camera
    can still join the AP; receiving GameController UDP broadcast needs
    zero egress. We therefore drop *every* outbound IP packet on the
    wifi interface. `None` (no profile active) renders nothing - egress
    stays governed by the OUTPUT ACCEPT policy.
    """
    if not wifi_lock_iface:
        return "# (no field-wifi profile active - wifi egress not locked)"
    return f"-A OUTPUT -o {wifi_lock_iface} -j DROP"


def _render_v4(policy: FirewallPolicy, local_v4: list[str], wifi_lock_iface: str | None) -> str:
    ...
    `wifi_lock_iface` is the field-wifi interface to lock to
    receive-only, or None when no field profile is active.
    """
    gated: list[str] = []
    if policy.wildcard:
    ...
    wifi_block = _render_wifi_egress_block(wifi_lock_iface)
...
:FORWARD DROP [0:0]
:OUTPUT ACCEPT [0:0]
# Flush only the chains we own. Applied with `iptables-restore --noflush`,
# so INPUT and OUTPUT are the chains we own: this clears prior AICam rules
# atomically (no duplicate accumulation across re-applies) while leaving
# FORWARD and any other filter chains (Docker, ufw) and the nat table
# untouched. OUTPUT keeps its ACCEPT policy, so flushing it leaves all
# non-wifi egress open; the only OUTPUT rule we add is the field-wifi
# receive-only lock below.
-F INPUT
-F OUTPUT
...
# Field-wifi receive-only egress lock. Drops all outbound IP on the
# field interface while a profile is active; never touches eth0 / lo
# egress (OUTPUT policy stays ACCEPT for those).
{wifi_block}
COMMIT
"""


def _render_v6(policy: FirewallPolicy, local_v6: list[str], wifi_lock_iface: str | None) -> str:
    ...
    `wifi_lock_iface` locks the field-wifi interface to receive-only
    (the field link runs IPv4-only, so this drops all v6 egress on it)
    or None when no field profile is active.
    """
    gated: list[str] = []
    if policy.wildcard:
        gated.append(f"-A INPUT -p tcp --dport {SSH_PORT} -j ACCEPT")
...
:FORWARD DROP [0:0]
:OUTPUT ACCEPT [0:0]
# Flush the chains we own; see the IPv4 renderer.
-F INPUT
-F OUTPUT
...
# Field-wifi receive-only egress lock.
{wifi_block}
COMMIT
"""
...
def render_ruleset(
    ...
    `wifi_lock_iface` is the field-wifi interface to lock to
    receive-only, or None to leave egress unrestricted.
    """
    return (
        _render_v4(policy, local_v4 or [], wifi_lock_iface),
        _render_v6(policy, local_v6 or [], wifi_lock_iface),
    )
...
def render_from_config(
    ...
    GameController port opens to. `wifi_lock_iface` locks the
    field-wifi interface to receive-only.
    """
    return render_ruleset(parse_allowed_ip_ranges(raw), local_v4, local_v6, wifi_lock_iface)
